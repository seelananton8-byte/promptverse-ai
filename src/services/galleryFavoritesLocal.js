...
export const saveGalleryFavoriteLocal = ({
  prompt,
  response,
}) => {
  try {
...
export const getGalleryFavoritesLocal = async () => {
  try {
    return JSON.parse(
      localStorage.getItem(STORAGE_KEY) || "[]"
    );
...
export const deleteGalleryFavoriteLocal = async (id) => {
  try {
...
export const clearAllGalleryFavoritesLocal = async () => {
  try {
